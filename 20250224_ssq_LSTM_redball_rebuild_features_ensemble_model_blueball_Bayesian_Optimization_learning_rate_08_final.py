...
from keras.models import Sequential  # 从Keras导入Sequential，用于构建顺序模型
from keras.layers import LSTM, Dense, Dropout, Input  # 从Keras导入LSTM、Dense、Dropout和Input层
...
# 定义模型构建函数
def create_lstm_model(neurons1=32, neurons2=16, neurons3=16, dropout1=0.1, dropout2=0.1, dropout3=0.1,optimizer='adam', learning_rate=0.001):
    # 定义LSTM模型结构
    if optimizer == 'adam':
    ...
    model = Sequential([  # 使用顺序模型
        Input(shape=(window, X.shape[2])),  # 输入层，输入形状为(window_size, 特征数)
        LSTM(neurons1, return_sequences=True),  # 第一层LSTM，返回整个序列
        Dropout(dropout1),  # 第一层Dropout，用于防止过拟合
        LSTM(neurons2, return_sequences=True),  # 第二层LSTM，返回整个序列
        Dropout(dropout2),  # 第二层Dropout
        LSTM(neurons3, return_sequences=False),  # 第三层LSTM，只返回最后一步的输出
        Dropout(dropout3),  # 第三层Dropout
        Dense(64, activation='relu'),  # 全连接层，64个神经元，激活函数为ReLU
        Dense(6, activation='linear')  # 输出层，6个神经元，激活函数为线性（用于回归）
    ])
    model.compile(loss='mse', optimizer=opt, metrics=['mae'])  # 编译模型，损失函数为MSE，指标为MAE
    return model  # 返回模型对象

# 定义超参数范围搜索函数
def objective(trial):
    # 定义Optuna的目标函数，用于超参数优化
    neurons1 = trial.suggest_int('neurons1', 16, 256)  # 优化第一层LSTM的神经元数量
    neurons2 = trial.suggest_int('neurons2', 8, 128)  # 优化第二层LSTM的神经元数量
    neurons3 = trial.suggest_int('neurons3', 8, 64)  # 优化第三层LSTM的神经元数量
    dropout1 = trial.suggest_float('dropout1', 0.0, 0.5)  # 优化第一层Dropout的比率
    dropout2 = trial.suggest_float('dropout2', 0.0, 0.5)  # 优化第二层Dropout的比率
    dropout3 = trial.suggest_float('dropout3', 0.0, 0.5)  # 优化第三层Dropout的比率
    optimizer = trial.suggest_categorical('optimizer', ['adam', 'rmsprop', 'Nadam'])  # 优化优化器类型
    learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-1)  # 学习率
    batch_size = trial.suggest_categorical('batch_size', [32, 64, 128])  # 优化批量大小
    epochs = trial.suggest_int('epochs', 50, 200)  # 优化训练轮数

    regressor = KerasRegressor(model=create_lstm_model, epochs=epochs, batch_size=batch_size, verbose=0,
                               neurons1=neurons1, neurons2=neurons2, neurons3=neurons3, dropout1=dropout1,
                               dropout2=dropout2, dropout3=dropout3,optimizer=optimizer, learning_rate=learning_rate)
    # 使用KerasRegressor包装LSTM模型

    tscv = TimeSeriesSplit(n_splits=3)  # 定义时间序列交叉验证，划分为3折
    scores = []  # 初始化评分列表
    for train_index, val_index in tscv.split(X):
        ...
        scores.append(regressor.score(X_val, y_val))  # 计算验证集的评分

    return np.mean(scores)  # 返回验证集评分的平均值
...
# 使用最佳参数训练最终模型
best_params = {k: v for k, v in best_trial.params.items() if k in create_lstm_model.__code__.co_varnames}
'''
...
# 提取与模型构建函数参数匹配的最佳超参数
model_lstm = create_lstm_model(**best_params)  # 使用最佳参数构建LSTM模型
...
print("LSTM Model Evaluation:", y_pred_lstm_rescaled)
print('LSTM Model Evaluation:', y_test_rescaled)
...
mape_lstm = mean_absolute_percentage_error(y_test_rescaled, y_pred_lstm_rescaled)
...
# 绘制每一组实际值和预测值
for i in range(6):
    plt.plot(y_test_rescaled[:, i], label=f'Actual Group {i + 1}')
    plt.plot(y_pred_lstm_rescaled[:, i], label=f'Predicted Group {i + 1}', linestyle='--')

plt.title('LSTM Model Predictions vs Actual for 6 Groups')
plt.xlabel('Sample Index')
plt.ylabel('Red Ball Numbers')
plt.legend()
plt.grid(True)
...
num = num[0] + 1
plt.savefig(f'{num}红球在测试集上的预测值.png', bbox_inches='tight', dpi=300)
plt.close()
# plt.show()


# 准备模型输入数据
scaled_data = scaled_red[-window:]  # 获取最后window个时间步的归一化数据
lstm_input = scaled_data.reshape(1, window, 6)  # 调整形状为(1, window_size, 特征数)

# 预测红球
pred_front = model_lstm.predict(lstm_input).reshape(-1)  # 使用模型预测，并调整预测结果形状
pred_front_rescaled = scaler.inverse_transform([pred_front])[0]  # 对预测结果进行反归一化
front_balls = list(map(int, np.round(pred_front_rescaled)))  # 将预测结果转换为整数列表
print("Predicted front balls:", front_balls)
