...
由 pipeline.py 在每个开奖日使用，记录编排器各步骤的执行状态。
状态持久化到 JSON 文件，支持：
  - 跨进程状态共享（pipeline.py 与各步骤子进程）
  - 异常中断后的状态可读
  - 微信推送汇总

对标 019_etf_daily_sync_and_backtest/pipeline_status.py 的完整模式。
"""
...
# ── 状态文件路径 ──
_DEFAULT_STATUS_PATH = Path(__file__).parent / "output" / "pipeline_status.json"
...
def push_pipeline_summary(
    ...
    Args:
        ...
        notify_func: 推送函数，签名 notify_func(title, content)。
    """
    steps = status_data.get("steps", {})
    lines = [f"🔴 双色球预测管线 | {status_data.get('date', '')}"]
    ...
    notify_func(
        title=f"🔴 双色球管线 | {status_data.get('date', '')}",
        content="\n".join(lines),
    )
